...
public class SignupController {
	...
	@FXML
	private TextField address1Field, address2Field, cityField, provinceField, postalCodeField, countryField;
	@FXML
	private TextField cardNameField, cardNumberField, expMonthField, expYearField, securityCodeField, cardPostalField,
			cardCountryField;

	@FXML
	private Label errorFirstName, errorLastName, errorEmail, errorConfirmEmail;
	@FXML
	private Label errorPassword, errorConfirmPassword, errorAddress1, errorCity;
	@FXML
	private Label errorProvince, errorZip, errorCountry;
	@FXML
	private Label errorCardName, errorCardNumber, errorExp, errorCvv, errorCardZip, errorCardCountry;
	...
	public void handleSignup() {
		...
		if (!isValidPassword(passwordField.getText())) {
			errorPassword.setText(
					"The password must be over 8 characters long and include an uppercase letter, a lowercase letter, and a special character.");
			valid = false;
		...
		// Save to DB (you can expand this part to store contact/payment if needed)
		if (registerUser(emailField.getText(), passwordField.getText())) {
	...
	/*
	 * private boolean registerUser(String email, String password) { String query =
	 * "INSERT INTO CR_User (username, password, role, email) VALUES (?, ?, ?, ?)";
	 * try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement
	 * stmt = conn.prepareStatement(query)) { stmt.setString(1, email); // using
	 * email as username stmt.setString(2, hashPassword(password));
	 * stmt.setString(3, "Customer"); stmt.setString(4, email);
	 * stmt.executeUpdate(); return true; } catch (SQLException e) {
	...
	private boolean registerUser(String email, String password) {
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			stmt.setString(1, email); // username
